...
where ${\bf y} = (Y_1,...,Y_n)'$ is the vector of responses, $\beta = (\beta_0, \beta_1,...,\beta_k)'$ is the vector of regression coefficients, ${\bf X}$ is the design matrix (remember that the first column of ${\bf X}$ is a column of 1s!), and $\varepsilon = (\varepsilon_1,...,\varepsilon_n)'$ is the vector of error terms.
...
To capture this kind of structure, suppose that there are $m$ independent groups, and $n_i$ observations for each group $i=1,...,m$. Let ${\bf y}_i = (Y_{i,1},...,Y_{i,n_i})'$ denote the vector of responses for group $i$, and let ${\bf X}_i$ denote the design matrix for group $i$. Within each group, we could still imagine a linear model:
...
* $g(\mu_i) = {\bf X}_i \beta$ 
    ...
    * Note that $\mu_i = (\mu_{i,1},...,\mu_{i,n_i})'$ is a vector. By $g(\mu_i)$, we mean "apply the function $g$ to each element of the vector", i.e. $g(\mu_i) = (g(\mu_{i,1}),...,g(\mu_{i,n_i}))'$
* $Var({\bf y}_i | {\bf X}_i) = \phi {\bf V}_i^{1/2} {\bf R}_i(\rho) {\bf V}_i^{1/2}$
...
$$\Psi(\beta) = \sum \limits_{i=1}^m {\bf D}_i' {\bf W}_i^{-1} ({\bf y}_i - \mu_i) = 0$$
...
$${\bf H}(\beta) := \frac{\partial}{\partial \beta}\Psi(\beta) = -\sum \limits_{i=1}^m {\bf D}_i' {\bf W}_i^{-1} {\bf D}_i$$

The coefficients $\beta$, and the additional parameters $\rho$ and $\phi$, are then simultaneously estimated as follows. 

### The algorithm

Here let $N = \sum \limits_{i=1}^m n_i$ be the total number of observations across all groups, and $k + 1$ the number of coefficients to estimate (i.e., $\beta = (\beta_0,...,\beta_k)'$).
...
$$\beta^{(0)} = \left( \log \left( \frac{\bar{Y}}{1 - \bar{Y}} \right), 0, 0, ..., 0 \right)'$$
...
$$\beta^{(0)} = \left( \log (\bar{Y}), 0, 0, ..., 0 \right)'$$
...
* $\widehat{{\bf A}} = \sum \limits_{i=1}^m \widehat{\bf D}_i' \widehat{\bf W}_i^{-1} \widehat{\bf D}_i$
* $\widehat{{\bf B}} = \sum \limits_{i=1}^m \widehat{\bf D}_i' \widehat{\bf W}_i^{-1} ({\bf y}_i - \widehat{\mu}_i)({\bf y}_i - \widehat{\mu}_i)' \widehat{\bf W}_i^{-1} \widehat{\bf D}_i$
...
* You are welcome, and encouraged, to write helper functions that are called by `geeC`
...
* Your C++ code must be stored in a file called `gee_code.cpp`
* The `geeC` function must be callable from R, after sourcing `gee_code.cpp`
