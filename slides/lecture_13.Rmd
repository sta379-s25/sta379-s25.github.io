...
* **Nonlinear least squares:** Minimize $L({\bf y}, \beta) = \sum \limits_{i=1}^n (Y_i - m(X_i, \beta))^2$ for some nonlinear function $m$

* **Logistic regression:** Minimize $L({\bf y}, \beta) = - \sum \limits_{i=1}^n \left \lbrace Y_i(\beta_0 + \beta_1 X_i) - \log(1 + e^{\beta_0 + \beta_1 X_i}) \right\rbrace$

* **Robust regression:** Minimize $L({\bf y}, \beta) = \sum \limits_{i=1}^n \rho(Y_i - \beta_0 - \beta_1 X_i)$ where 
